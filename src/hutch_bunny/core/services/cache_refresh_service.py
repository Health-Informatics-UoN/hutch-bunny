...
class CacheRefreshService:
    ...
    def start(self) -> None:
        ...
        if self.settings.CACHE_REFRESH_ON_STARTUP:
            ...
            except Exception as e:  # noqa: BLE001 - keep startup resilient to any cache failure
        ...
        self.thread.start()
        logger.info(
            f"Cache refresh service started (interval: {self.settings.CACHE_TTL_HOURS} hours)"
        )
    ...
    def _refresh_loop(self) -> None:
        ...
        while self.running:
            try:
                ...
                next_refresh = self.last_refresh + timedelta(
                    hours=self.settings.CACHE_TTL_HOURS
                )

                if datetime.now(UTC) >= next_refresh:
                    logger.info(
                        f"{datetime.now(UTC) - self.last_refresh} elapsed since last refresh: Starting scheduled cache refresh"
                    )
                    self._refresh_cache()
                    self.last_refresh = datetime.now(UTC)
            ...
            except Exception as e:  # noqa: BLE001 - background loop must survive any single-iteration failure
    ...
    def _refresh_cache(self) -> None:
        ...
        for query in queries:
            for modifiers in modifier_sets:
                try:
                    logger.debug(
                        f"Refreshing {query['code']} with modifiers {modifiers}"
                    )
                    execute_query(
                        query, modifiers, db_client=db_client, settings=self.settings
                    )
                except Exception as e:  # noqa: BLE001 - one query's failure shouldn't block the others
