...
class DistributionCacheService:
    ...
    def _generate_cache_key(self, query_dict: dict, modifiers: list) -> str:
        ...
        cache_data = {"query": query_dict, "modifiers": modifiers}
    ...
    def _is_cache_valid(self, cache_path: Path) -> bool:
        ...
        file_time = datetime.fromtimestamp(cache_path.stat().st_mtime, tz=UTC)
        expiry_time = file_time + timedelta(hours=self.ttl_hours)
        return datetime.now(UTC) < expiry_time

    def get(
        self, query_dict: dict[str, object], modifiers: list
    ) -> RquestResult | None:
        ...
        if self._is_cache_valid(cache_path):
            ...
            except Exception as e:  # noqa: BLE001 - a corrupt/unreadable cache file shouldn't break the query
    ...
    def set(
        self, query_dict: dict[str, object], modifiers: list, result: RquestResult
    ) -> None:
        ...
        except Exception as e:  # noqa: BLE001 - a failed cache write shouldn't fail the query
    ...
    def clear(self) -> None:
        ...
        for cache_file in self.cache_dir.glob("*.json"):
            try:
                cache_file.unlink()
            except Exception as e:  # noqa: BLE001 - keep clearing the rest of the cache even if one file fails
