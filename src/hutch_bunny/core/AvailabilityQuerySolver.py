...
class AvailabilityQuerySolver:
    ...
    def _solve_rules(self) -> int:
            ...
            for current_group in self.query.cohort.groups:
                ...
                for current_rule in current_group.rules:
                    ...
                    if current_rule.varcat != "Person": # i.e condition, observation, measurement or drug
                        ...
                        # for the presence of the concept. This is computationally more expensive, but is more
                        ...
                        if  (left_value_time is not None or right_value_time is not None) and time_category == "AGE":
                            ...
                            self.addRelativeAgeSearch(left_value_time, right_value_time)


                        """"
                        STANDARD CONCEPT ID SEARCH
                        """
                        self.addConceptSearch(current_rule)
                        ...
                        if left_value_time is not None and (
                            ...
                            self.addRelativeTime(left_value_time, right_value_time)
        ...
        return int(output[0])
    def addRelativeTime(self, left_value_time, right_value_time):
        ...
        if left_value_time == "":
            ...
            self.condition = self.condition.where(ConditionOccurrence.condition_start_date >= relative_date)
            self.drug = self.drug.where(DrugExposure.drug_exposure_start_date >= relative_date)
        else:
            self.meas = self.meas.where(Measurement.measurement_date <= relative_date)
            self.obs = self.obs.where(Observation.observation_date <= relative_date)
            self.condition = self.condition.where(ConditionOccurrence.condition_start_date <= relative_date)
            self.drug = self.drug.where(DrugExposure.drug_exposure_start_date <= relative_date)
    def addConceptSearch(self, current_rule: Rule):
        if current_rule.operator == "=":
            condition = self.condition.where(ConditionOccurrence.condition_concept_id == int(current_rule.value))
            drug = self.drug.where(DrugExposure.drug_concept_id == int(current_rule.value))
            meas = self.meas.where(Measurement.measurement_concept_id == int(current_rule.value))
            obs = self.obs.where(Observation.observation_concept_id == int(current_rule.value))
        else:
            condition = self.condition.where(ConditionOccurrence.condition_concept_id != int(current_rule.value))
            drug = self.drug.where(DrugExposure.drug_concept_id != int(current_rule.value))
            meas = self.meas.where(Measurement.measurement_concept_id != int(current_rule.value))
    ...
    def addRelativeAgeSearch(self, left_value_time: str, right_value_time: str):
        # due to the way the query is expressed and how split above, if the left value is empty
        # it indicates a less than search
        if left_value_time == "":
            self.condition = self.condition.where (extract('year', ConditionOccurrence.condition_start_date) - extract('year', Person.birth_datetime) < int (right_value_time))
            self.drug = self.drug.where (extract('year', DrugExposure.drug_exposure_start_date) - extract('year', Person.birth_datetime) < int (right_value_time))
            self.meas = self.meas.where(extract('year', Measurement.measurement_date) - extract('year', Person.birth_datetime) < int (right_value_time))
            self.obs = self.obs.where(extract('year', Observation.observation_date) - extract('year', Person.birth_datetime) < int (right_value_time))
        else:
            self.condition = self.condition.where(extract('year', ConditionOccurrence.condition_start_date) - extract('year', Person.birth_datetime) > int (left_value_time))
            self.drug = self.drug.where  (extract('year', DrugExposure.drug_exposure_start_date) - extract('year', Person.birth_datetime) > int (left_value_time))
            self.meas = self.meas.where(extract('year', Measurement.measurement_date) - extract('year', Person.birth_datetime) > int  (left_value_time))
            self.obs = self.obs.where(extract('year', Observation.observation_date) - extract('year' , Person.birth_datetime) > int (left_value_time))
