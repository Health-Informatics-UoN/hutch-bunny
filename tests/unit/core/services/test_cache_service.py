...
def test_cache_get_set(mock_settings: Mock) -> None:
    ...
    modifiers = []
    result = RquestResult(uuid="test", status="ok", collection_id="test", count=100)
...
def test_cache_ttl_expiration(mock_settings: Mock) -> None:
    ...
    fake_stat = Mock(st_mtime=(datetime.now(UTC) - timedelta(hours=2)).timestamp())
    with patch(
        "hutch_bunny.core.services.cache_service.Path.stat", return_value=fake_stat
    ):
