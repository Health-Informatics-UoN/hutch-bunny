...
@pytest.fixture
def mock_daemon_settings(httpserver: HTTPServer) -> Generator[DaemonSettings, None, None]:
    ...
    os.environ["TASK_API_BASE_URL"] = httpserver.url_for("").rstrip("/")  # Set mock server URL without trailing slash
...
def test_daemon_task_processing(httpserver: HTTPServer, mock_daemon_settings: DaemonSettings) -> None:
    """
    ...
    def next_response(request: Request) -> Response:
        ...
        return Response("", status=204)  # Default to no content after queue is exhausted
    ...
    httpserver.expect_request(result_endpoint, method="POST").respond_with_handler(store_results)
