...
def test_first_query_computes_and_caches(
    ...
    mock_result = RquestResult(
        uuid="test-uuid", status="ok", collection_id="test-collection", count=100
    )
    ...
    result1 = execute_query(
        distribution_query, modifiers, mock_db_client, mock_settings
    )
...
def test_second_query_uses_cache(
    mock_solve: Mock, distribution_query: dict[str, str], mock_settings: Mock
) -> None:
    mock_result = RquestResult(
        uuid="test-uuid", status="ok", collection_id="test-collection", count=100
    )
...
def test_cache_service_with_polling(
    mock_execute: Mock, mock_get_db_client: Mock, mock_settings: Mock
) -> None:
...
def test_task_handler_uses_cache(mock_solve: Mock, mock_settings: Mock) -> None:
    mock_result = RquestResult(
        uuid="task-uuid", status="ok", collection_id="test_collection", count=150
    )
    mock_solve.return_value = mock_result
...
def test_background_refresh_updates_cache(
    ...
    with patch("time.sleep"):  # Speed up test
        ...
        with patch(
            "hutch_bunny.core.services.cache_refresh_service.datetime"
        ) as mock_datetime:
            ...
            should_refresh = (
                mock_datetime.now()
                >= cache_service.last_refresh
                + timedelta(hours=mock_settings.CACHE_TTL_HOURS)
            )
...
def test_system_stable_with_cache_errors(
    mock_get_db: Mock, mock_execute: Mock, mock_settings: Mock
) -> None:
    ...
    try:
        cache_service._refresh_cache()
    except Exception:  # noqa: BLE001 - verifying no exception escapes, of any type
